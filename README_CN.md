...
一个用于 API 开发的 IntelliJ IDEA 插件 —— 将 API 文档导出至 YApi/Postman/Hoppscotch/Markdown，发送请求，直接在代码中管理接口。
...
| Hoppscotch | ✓ | — | 可直接导入 Hoppscotch 的 JSON 文件 |
...
3. 选择目标格式（YApi / Postman / Hoppscotch / Markdown / cURL / HTTP Client）
4. API 将自动导出
...
- **ApiExporter** — 将 `ApiEndpoint` 模型转换为输出格式（YApi、Postman、Hoppscotch、Markdown、cURL、HTTP Client）
